...
import { 
  ...
  Route, Play, RefreshCw, X, ArrowLeft
} from 'lucide-react'
...
const Navigation3DComponent = dynamic(() => import('@/components/Navigation3DComponent'), { 
  ssr: false,
  loading: () => (
    <div className="w-full h-full bg-gray-100 flex flex-col items-center justify-center">
      ...
      <div className="text-gray-400 text-sm mt-2">Preparing navigation view</div>
    </div>
  )
})
...
// Sample data - realistic for a local supplier
  ...
  while (visited.size < pending.length) {
    ...
    if (nearest) {
      ...
      route.push({
        ...
        time: `${Math.round(actualDist * 3 + 5)} min` // Assume 3 min/km + 5 min stop
      })
...
export default function SupplierDashboard() {
  ...
  const totalRouteDistance = optimizedRoute.reduce((sum, stop) => {
    return sum + parseFloat(stop.distance)
  }, 0)
  const totalRouteTime = optimizedRoute.reduce((sum, stop) => {
    return sum + parseInt(stop.time)
  }, 0)
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          ...
          {activeTab === 'deliveries' && (
            <>
              ...
              {/* Regular Deliveries List (when not showing optimized route) */}
              {!showOptimizedRoute && (
                <div className="space-y-2">
      ...
      <main className="flex-1 relative">
        ...
        {isNavigating && optimizedRoute.length > 0 ? (
          ...
          {/* Route Summary Bar - Show when optimized */}
