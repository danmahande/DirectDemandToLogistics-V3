...
// High-quality satellite tile sources
const SATELLITE_TILE_SOURCES = {
  // ESRI Clarity - Free, high quality satellite
  esri: {
    url: 'https://clarity.maptiles.arcgis.com/arcgis/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
    maxzoom: 19,
    attribution: 'Esri'
  },
  // ESRI World Imagery - Alternative
  esriWorld: {
  ...
  sentinel: {
    url: 'https://tiles.maps.eox.at/wms?service=wms&request=GetMap&version=1.1.1&layers=s2cloudless-2020_3857&format=image/jpeg&width=256&height=256&srs=EPSG:3857&bbox={bbox}',
    maxzoom: 18,
    attribution: 'EOX'
  }
}
...
class TileEnhancer {
  ...
  private enhancementLevel: number = 1.0
  ...
  setEnhancementLevel(level: number) {
    this.enhancementLevel = Math.max(0, Math.min(2, level))
  }

  // Apply client-side enhancement using Canvas 2D
  async enhanceImage(imageData: ImageData): Promise<ImageData> {
    if (!this.ctx || this.enhancementLevel === 0) {
      return imageData
    }
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      const saturation = 1 + (level * 0.2)
      r = gray + (r - gray) * saturation
      g = gray + (g - gray) * saturation
      b = gray + (b - gray) * saturation

      // 3. Adjust brightness
      const brightness = level * 10
      r += brightness
      g += brightness
      b += brightness

      // 4. Vibrance (smart saturation for less saturated colors)
      const maxChannel = Math.max(r, g, b)
      const minChannel = Math.min(r, g, b)
      const currentSat = maxChannel > 0 ? (maxChannel - minChannel) / maxChannel : 0
      const vibranceBoost = level * 0.1 * (1 - currentSat)
      
      r += (r - gray) * vibranceBoost
      g += (g - gray) * vibranceBoost
      b += (b - gray) * vibranceBoost
...
export default function Navigation3DComponent({
  ...
  const tileEnhancerRef = useRef<TileEnhancer | null>(null)
  ...
  // Initialize tile enhancer
  useEffect(() => {
    tileEnhancerRef.current = new TileEnhancer()
    tileEnhancerRef.current.setEnhancementLevel(enhancementLevel)
  }, [])

  // Update enhancement level
  useEffect(() => {
    if (tileEnhancerRef.current) {
      tileEnhancerRef.current.setEnhancementLevel(enhancementLevel)
    }
  }, [enhancementLevel])
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        ...
        // Choose tile source based on settings
        const tileSource = useEnhancedTiles ? SATELLITE_TILE_SOURCES.esri : SATELLITE_TILE_SOURCES.esriWorld

        // ============================================
        // CREATE MAP WITH HIGH-QUALITY SATELLITE TILES
        // ============================================
        ...
        <>
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-20">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setUseEnhancedTiles(!useEnhancedTiles)}
          ...
          {useEnhancedTiles && (
            <div className={`absolute top-20 right-4 p-3 rounded-lg shadow-lg z-20 ${isNightMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className={`text-xs font-medium mb-2 ${isNightMode ? 'text-gray-300' : 'text-gray-600'}`}>
                Enhancement: {Math.round(enhancementLevel * 100)}%
              </div>
              <input
                type="range"
                min="0"
